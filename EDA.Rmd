...
output3 = table1(~ sex + age + obstruct + perfor + adhere + differ + extent + surg + node4| rx,
                 data = colon_tb %>% filter(etype == 1), 
                 render.continuous = my.render.cont, 
                 render.categorical = my.render.cat,
                 render.missing = NULL, overall = FALSE) 
...
```{r, message = FALSE, warning = FALSE}
colon_ass = colon %>% 
  mutate(death = ifelse(etype == 2, 1, 0),
         recur = ifelse(etype == 1, 1, 0)) %>% 
  mutate(comp = ifelse())
...
colon %>% subset(etype == 2)
  survival::hr_plot(dependent, explanatory, dependent_label = "Survival")
```
